...
function SimpleTablePanel({ type, result }) {
  const configs = {
    ...
    payments: { title: 'Payments', subtitle: 'Order payment records and collection status.', rows: result?.payments || [], columns: [{ key: 'transaction_id', label: 'Transaction' }, { key: 'order_id', label: 'Order' }, { key: 'customer_name', label: 'Customer' }, { key: 'amount', label: 'Amount', render: (row) => formatCurrency(row.amount) }, { key: 'status', label: 'Status', render: (row) => <StatusBadge status={row.status} /> }, { key: 'date', label: 'Date', render: (row) => formatDateTime(row.date) }] },
